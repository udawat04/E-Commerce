...
const categoryController = {
  async store(req, res, next) {
    ...
    try {
      ...
      add = await Category.create({
        category,
        image: "upload/category/image/" + req.file.filename,
      });
    } catch (error) {
  ...
  async update(req, res) {
    ...
    try {
      ...
      prod = await Category.findByIdAndUpdate(
        { _id: id },
        { subcategory },
        { new: true }
      );
  ...
  async getCatWithSubCats(req, res, next) {
    let display;
    try {
      display = await Category.aggregate([
        {
          $lookup: {
            from: "subcategories",
            localField: "_id",
